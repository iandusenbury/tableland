...
function randLat() {
  return Math.floor(Math.random() * 12 + 35)
}
function randLong() {
  return Math.floor(Math.random() * 12 - 100)
}
function randEndTime(start) {
  return Math.floor(Math.random() * 50 + start)
}
function randPrio() {
...
const buildMap = () => {
  const markers = nodes.map((node, index) => (
    <Marker key={index} label={index.toString()} position={node.location} />
  ))
